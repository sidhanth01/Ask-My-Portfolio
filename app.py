...
with st.sidebar:
    ...
    resume_files = [f for f in os.listdir(DATA_PATH) if f.lower().endswith('.pdf')]
    if resume_files:
        resume_filename = resume_files[0]
        resume_path = os.path.join(DATA_PATH, resume_filename)
        with open(resume_path, "rb") as f:
            resume_bytes = f.read()
            # Embed PDF using base64 for direct display in an iframe
            ...
            # Offer download via st.download_button for reliability
            st.download_button(
                label=f"⬇️ Download {resume_filename}",
                data=resume_bytes,
